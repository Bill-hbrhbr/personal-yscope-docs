...
This plan splits the search-coordinator MVP into reviewable PRs. It uses the `search-coordinator/init` branch as a prototype, but does not assume that the prototype's commits are the right merge boundaries. Each PR should leave the repository buildable and include tests for the behavior it introduces.

The authoritative scope is Section 4 of `search-coordinator.md`. The underscore-prefixed legacy appendix in that document is reference material and must not expand the MVP accidentally.
...
Like compression, the graph has a TDL termination task. Per-archive search tasks write results through clp-s and return a small completion payload; `search::commit` consumes those graph outputs, looks up the CLP query job by the Spider ID, and transactionally changes `RUNNING` to `SUCCEEDED`. The coordinator owns dispatch and failure reporting and verifies the committed status after a successful graph.
...
- `search::commit` uses the same row lock and `RUNNING`-to-`SUCCEEDED` CAS as `compression::commit`. Coordinator failure handling first preserves an already-committed success.
- A single coordinator deployment and an explicit legacy-scheduler cutover are required so two unguarded writers cannot own the same plain-search row.
- Persist the minimum lifecycle status and supporting columns needed by external consumers and fault-tolerant recovery. Keep reconstructible phases such as graph construction, Spider polling, and commit verification local to `QueryJobHandle`; do not expand `QueryJobStatus` or write MySQL on every local phase change. This reduces unnecessary updates and row contention. Add a durable field only when it closes a crash-recovery ambiguity, as `spider_id` does for reattachment and duplicate-submission prevention.

## Starting point: `search-coordinator/init`

The prototype already contains:

- A `search-coordinator` workspace crate, binary entry point, configuration loading, database setup, signal handling, and package-image wiring.
- Matching Python and Rust `SearchCoordinator` configuration models and a validator requiring Spider when the coordinator is enabled.
...
- No `search::commit` TDL task exists, so successful Spider completion cannot transactionally update the SQL job to `SUCCEEDED` with duration.
- Terminal handling does not yet verify a successful commit or preserve it when Spider subsequently reports a failure.
...
Test default and invalid values, Python/Rust field parity, and the requirement that `search_coordinator` cannot be enabled without Spider. Keep separately named retry and timeout policies for the per-archive search task and the `search::commit` termination task; correct the prototype's wiring so commit timeouts are not used as search-task timeouts.
...
- On terminal Spider success, verify that `search::commit` already wrote `SUCCEEDED`; on failure, write `FAILED`, duration, and `status_msg` only if a successful commit did not already win.
- Report setup and polling failures without overwriting an already-terminal state.

The success transition belongs to `search::commit` and uses a row lock plus a `RUNNING`-to-`SUCCEEDED` CAS. Tests should exercise repeated completion checks, already-terminal jobs, oversized task counts, duration/status messages, and best-effort failure reporting. Unexpected Spider cancellation is a failure for MVP; it does not introduce `KILLED` or coordinator-driven cancellation semantics.
...
Port the minimum archive selection needed by plain search: dataset selection, time-range filtering, retention cutoff, deterministic/newest-first ordering, and any required archive batching. Produce an in-memory per-archive Spider plan but do **not** create `query_tasks` rows or track `num_tasks_completed`.

Specify non-overlapping ownership with the legacy scheduler. Unsupported rows must not be claimed, while accepted plain-search rows must not also be consumed by the legacy path. Test mixed types, malformed configs, filters, and empty archive sets.

### PR 3 — `clp-tdl-package` search and commit tasks

Follow the existing compression task's package/runtime structure:

- Add serializable per-archive search inputs/options under `clp-rust-utils::task_io::search`, plus a minimal `SearchTaskOutput` that echoes the archive identity for termination-task validation. It carries no search hits; clp-s writes those directly to MongoDB.
- Add `task/search/{mod.rs,search.rs,commit.rs}`: keep the `#[task]` wrappers thin and put testable implementations in `search.rs` and `commit.rs`.
- Register `search::search` and `search::commit` in `clp-tdl-package/src/lib.rs` and document both in the package README.
...
The `search::commit` termination task receives the graph outputs through `TaskContext`, validates/deserializes them, and uses `ctx.job_id` to find the CLP query job by `spider_id`. Using DB configuration from `SpiderTaskExecutorConfig` and credentials from `CLP_DB_USER`/`CLP_DB_PASS`, it locks the job row and transactionally CAS-transitions `RUNNING` to `SUCCEEDED` with a DB-clock-derived duration. It is idempotent when the row is already `SUCCEEDED`; it neither writes `query_tasks` nor touches MongoDB.

Test argument construction, plain search options, malformed output, subprocess failure, process cleanup, results-cache configuration, output validation, commit idempotency, wrong-state refusal, rollback, and DB credential loading. Validate that the release build places both registered TDL functions in the Spider worker library at the path Spider loads.

### PR 4 — Implement `submit_query_job`

Replace the submitter's `todo!()` with Spider graph construction and job registration. Mirror the useful portions of `submit_s3_compression_job`: keep task-name constants synchronized with the TDL package, use typed inputs, serialize value inputs with MessagePack, and create one search task per planned archive. Attach `search::commit` as the graph's termination task and apply its own retry and soft/hard timeout policy.

Persist the Spider ID and transition the query job to `RUNNING` only after registration succeeds. Define retry/recovery behavior for the window between Spider registration and SQL persistence so retries do not silently create duplicate work. Explicitly define zero-archive behavior.
...
Exercise empty searches, partial task failure, unexpected Spider cancellation, SQL/commit failure, duplicate/retried submission, max-results behavior, and restart at each persistence boundary. Assert that a coordinator-side failure cannot overwrite committed success and that no `query_tasks` rows or non-results-cache output paths are used.
...
- Make commit and terminal handling idempotent if clp-s wrote results before restart or `search::commit` succeeded before the coordinator restarted.
...
- Keep MVP-only `search_coordinator` defaults in `tools/deployment/package-helm/values.yaml` and render the full block from `templates/configmap.yaml` only when Spider is enabled. Render distinct search and commit task execution policies.
- Add a search-coordinator Deployment modeled on the compression coordinator, including DB credentials, DB/Spider-storage readiness init containers, ConfigMap mount, `RUST_LOG`, telemetry environment, termination grace, scheduling, and resources.
...
- Add `searchCoordinator` values under Helm `scheduling` and `resources`, plus a Helm-only log-level value if used by the Deployment.
- Do not increment the Helm chart version on this MVP branch.

Docker/package integration:

- Verify the package image contains `/opt/clp/bin/search-coordinator` and the generated config contains the matching section.
...
- Ensure the Spider-worker image build depends on `clp-tdl-package` and copies the library containing the registered search and commit tasks to Spider's configured TDL package directory. Supply its config plus `CLP_DB_USER` and `CLP_DB_PASS`, since `search::commit` opens the MySQL connection itself.
- Wire package-controller/config-template enablement so the coordinator is started only with Spider and the legacy scheduler no longer claims supported plain-search jobs.
