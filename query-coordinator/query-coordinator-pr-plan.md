...
SQL query job
  ...
  -> Spider query::commit task: transactionally mark SQL job SUCCEEDED
  -> query coordinator: verify success or record graph failure
```

Like compression, the graph has a TDL termination task. Per-archive query tasks write results through clp-s and return a small completion payload; `query::commit` consumes those graph outputs, looks up the CLP query job by the Spider ID, and transactionally changes `RUNNING` to `SUCCEEDED`. The coordinator owns dispatch and failure reporting and verifies the committed status after a successful graph.
...
- `query::commit` uses the same row lock and `RUNNING`-to-`SUCCEEDED` CAS as `compression::commit`. Coordinator failure handling first preserves an already-committed success.
...
- No `query::commit` TDL task exists, so successful Spider completion cannot transactionally update the SQL job to `SUCCEEDED` with duration.
...
Test default and invalid values, Python/Rust field parity, and the requirement that `query_coordinator` cannot be enabled without Spider. Keep separately named retry and timeout policies for the per-archive query task and the `query::commit` termination task; correct the prototype's wiring so commit timeouts are not used as query-task timeouts.
...
Specify non-overlapping ownership with the legacy scheduler. Unsupported rows must not be claimed, while accepted non-aggregation query rows must not also be consumed by the legacy path. Test mixed types, malformed configs, filters, and empty archive sets.

### PR 3 — `clp-tdl-package` query and commit tasks

Follow the existing compression task's package/runtime structure:

- Add serializable per-archive query inputs/options under `clp-rust-utils::task_io::query`, plus a minimal `QueryTaskOutput` that echoes the archive identity for termination-task validation. It carries no search hits; clp-s writes those directly to MongoDB.
- Add `task/query/{mod.rs,search.rs,commit.rs}`: keep the `#[task]` wrappers thin and put testable implementations in `search.rs` and `commit.rs`.
- Register `query::clp_s_search` and `query::commit` in `clp-tdl-package/src/lib.rs` and document both in the package README.
...
- Make commit and terminal handling idempotent if clp-s wrote results before restart or `query::commit` succeeded before the coordinator restarted.
...
- Keep MVP-only `query_coordinator` defaults in `tools/deployment/package-helm/values.yaml` and render the full block from `templates/configmap.yaml` only when Spider is enabled. Render distinct query and commit task execution policies.
...
- Ensure the Spider-worker image build depends on `clp-tdl-package` and copies the library containing the registered query and commit tasks to Spider's configured TDL package directory. Supply its config plus `CLP_DB_USER` and `CLP_DB_PASS`, since `query::commit` opens the MySQL connection itself.
...
| Transactional `query::commit` success | 3, 4, 5 |
